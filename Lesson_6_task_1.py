...
class TrafficLight:
    ...
    def __init__(self, red_time: int = 7, yellow_time: int = 2, green_time: int = 5):
        self.__timing = {"red": red_time, "yellow": yellow_time, "green": green_time}

    def running(self, color: str):
        if list(self.__timing.keys()).index(color) != self.__next_index:
        ...
        for second in range(timer):
            ...
            time.sleep(1)
        next_index = self.__next_index + 1
        self.__next_index = next_index if next_index < len(self.__timing) else 0
    def __repr__(self):
